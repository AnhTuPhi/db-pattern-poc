...
## Why these 10, and what's next

This is **batch 1** of a larger plan. These 10 cover query performance, JPA pitfalls, and isolation — the patterns that produce the most production fires for an Oracle + Spring Boot + JPA + QueryDSL fintech stack.

Subsequent batches will cover: locking & deadlock, connection-pool tuning, zero-downtime migration, read replicas, partitioning, sharding, caching layers, JSONB, multi-tenancy, time-series, Oracle AWR/bind-peeking specifics, and ops/observability.
